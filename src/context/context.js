...
// Provider, Consumer _ GithubContext.Provider
...
const GithubProvider = ({ children }) => {
    ...
    const searchGithubUser = async (user) => {
        toggleError();
        setIsLoading(true)
        const respones = await axios(`${rootUrl}/users/${user}`).catch(err => console.log(err));
        console.log(error);
        if (respones) {
            setGithubUser(respones.data);
            const { login, followers_url } = respones.data;
            //repos
            //Followers
